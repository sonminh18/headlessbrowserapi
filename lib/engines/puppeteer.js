    ...
    return page.evaluate(() => {
        ...
        relativeVideoPatterns.forEach(({regex, type}) => {
            ...
            while ((relMatch = regex.exec(pageSource)) !== null) {
                let relativeUrl = relMatch[0].replace(/["']/g, '');
...
const clickVideoSourceButtons = async (page) => {
    return page.evaluate(() => {
        ...
        downloadSelectors.forEach(selector => {
            try {
                const elements = document.querySelectorAll(selector);
                elements.forEach(el => tryClick(el, selector, true));
            } catch (e) {}
        });
                ...
                if (hasVideoPlayer) {
                    ...
                    for (let pass = 0; pass < maxSourcePasses; pass++) {
                        ...
                        if (sourceClickResult.clicked > 0) {
                            ...
                            const newVideoSrc = await waitForVideoChange();
                            if (!newVideoSrc) {
                            ...
                            const primaryVideoUrl = await page.evaluate(() => {
                            ...
                            const newVideosFound = videoUrls.length - previousVideoCount;
                            if (newVideosFound > 0) {
                                util.Logging.info(`Pass ${pass + 1}: Found ${newVideosFound} new video(s), total: ${videoUrls.length}`);
                            }
                            previousVideoCount = videoUrls.length;
                            
                            // If MP4 source was clicked/active and we have MP4 video, stop immediately
                            // This prevents clicking other buttons that might switch away from MP4
                            const hasMp4Video = videoUrls.some(v => (v.url || '').toLowerCase().includes('.mp4'));
                            if (mp4SourceActivated && hasMp4Video) {
                                util.Logging.info(`MP4 source active and MP4 video found, stopping to preserve MP4 source`);
                                break;
                            }
                            
                            // If we have MP4 video even without explicit MP4 button, consider stopping
                            if (hasMp4Video && videoUrls.length >= 1) {
                                util.Logging.info(`Found MP4 video, stopping source detection to preserve it`);
                                break;
                            }
                        } else {
                            util.Logging.info(`Pass ${pass + 1}: No source buttons clicked`);
                            
                            // Even if no buttons clicked, extract current video URLs
                            const passVideoUrls = await extractVideoUrlsFromDOM(page);
                            passVideoUrls.forEach(videoInfo => {
                                videoInfo.mimeType = videoInfo.mimeType || getVideoMimeType(videoInfo.url);
                                videoInfo.size = videoInfo.size || 0;
                                ...
                                if (!videoUrls.some(v => v.url === videoInfo.url)) {
                                    videoUrls.push(videoInfo);
                                    util.Logging.info(`Pass ${pass + 1}: Found video URL from DOM: ${(videoInfo.url || '').substring(0, 80)}...`);
                                }
                            });
