...
/**
 * Parse an M3U8 playlist to extract video segments and quality variants
 * @param {string} m3u8Url - The URL of the M3U8 playlist
 * @param {Page} page - Puppeteer page to use for fetching
 * @returns {Promise<Array>} Array of video segment URLs and quality variants
 */
const parseM3U8Playlist = async (m3u8Url, page) => {
    try {
        // Fetch the M3U8 content using the page context to maintain cookies and headers
        ...
        if (!content) {
            util.Logging.warn(`Failed to fetch M3U8 content from ${m3u8Url}`);
            return [];
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Check for master playlist with quality variants
            if (line.startsWith('#EXT-X-STREAM-INF')) {
                results.masterPlaylist = true;
                isVariantPlaylist = true;
                
                // Parse attributes
                const attributes = {};
                const attrMatches = line.matchAll(/([A-Z-]+)=("[^"]*"|[^,]*)/g);
                ...
                currentVariant = {
                    ...
                    url: '' // Will be set in next iteration
                };
            ...
            // If we have a current variant and this line is not a tag, it's the variant URL
            if (isVariantPlaylist && currentVariant && !line.startsWith('#')) {
                // Resolve the URL if it's relative
                let variantUrl = line;
                if (!variantUrl.startsWith('http')) {
                    if (variantUrl.startsWith('/')) {
                        variantUrl = `${baseUrl.protocol}//${baseUrl.host}${variantUrl}`;
                    } else {
                        variantUrl = `${baseUrl.protocol}//${baseUrl.host}${basePath}${variantUrl}`;
                    }
                }
                
                currentVariant.url = variantUrl;
                results.variants.push(currentVariant);
                currentVariant = null;
                continue;
            }
            
            // Regular segment
            if (!line.startsWith('#') && !isVariantPlaylist) {
                // Resolve the URL if it's relative
                let segmentUrl = line;
                ...
                results.segments.push({
                    url: segmentUrl,
                    type: 'video/mp2t' // Default type for HLS segments
                });
            }
        }
        ...
        util.Logging.error(`Error parsing M3U8 playlist: ${error.message}`);
        return { masterPlaylist: false, variants: [], segments: [] };
    }
};
...
class PuppeteerSingleton {
    ...
    async renderWithOptions(req, res, next, options) {
        ...
        try {
            ...
            // Listen for response received events to detect video URLs
            client.on('Network.responseReceived', async (event) => {
                try {
                    ...
                    if ((isVideoExt && !isImageUrl(responseUrl)) || isVideoContentType || isManifestType) {
                        // Only collect the URL, not the content
                        const videoInfo = {
                            url: responseUrl,
                        ...
                        if (!videoUrls.some(v => v.url === responseUrl)) {
                            videoUrls.push(videoInfo);
                            util.Logging.info(`Detected video URL: ${responseUrl}`);
                        }
                    }
                } catch (err) {
            ...
            if (!isImage) {
                ...
                // Add delay if specified or use a default delay for video detection
                const videoDetectionDelay = delayTime || 3000; // Default 3 seconds for video to load
                ...
                // Process M3U8 playlists to extract video segments
                const hlsPlaylists = videoUrls.filter(v => 
                    v.url && v.url.toLowerCase().includes('.m3u8') || 
                    (v.mimeType && (v.mimeType.includes('mpegURL') || v.mimeType.includes('apple')))
                );
                
                if (hlsPlaylists.length > 0) {
                    util.Logging.info(`Found ${hlsPlaylists.length} HLS playlists, parsing for video segments`);
                    
                    for (const playlist of hlsPlaylists) {
                        try {
                            const parsedPlaylist = await parseM3U8Playlist(playlist.url, page);
                            
                            // Add HLS metadata to the playlist entry
                            playlist.isHLS = true;
                            playlist.hlsType = parsedPlaylist.masterPlaylist ? 'master' : 'media';
                            
                            // Add quality variants if this is a master playlist
                            if (parsedPlaylist.masterPlaylist && parsedPlaylist.variants.length > 0) {
                                playlist.variants = parsedPlaylist.variants;
                                
                                // Add each variant as a separate video URL
                                for (const variant of parsedPlaylist.variants) {
                                    if (!videoUrls.some(v => v.url === variant.url)) {
                                        videoUrls.push({
                                            url: variant.url,
                                            mimeType: 'application/x-mpegURL',
                                            isHLS: true,
                                        ...
                                        util.Logging.info(`Added HLS variant: ${variant.url} (${variant.resolution || 'unknown resolution'})`);
                                    }
                                }
                            }
                            
                            // Add individual segments if this is a media playlist
                            if (parsedPlaylist.segments.length > 0) {
                                playlist.segments = parsedPlaylist.segments;
                                
                                // We don't add individual segments to videoUrls as there could be hundreds
                                util.Logging.info(`Found ${parsedPlaylist.segments.length} segments in HLS playlist: ${playlist.url}`);
                            }
                        } catch (error) {
                            util.Logging.error(`Error processing M3U8 playlist ${playlist.url}: ${error.message}`);
                        }
                    }
                }
