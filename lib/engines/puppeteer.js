    ...
    async renderWithOptions(req, res, next, options) {
        ...
        try {
            ...
            page.on('request', request => {
                ...
                // Special handling for Google-specific video URLs (Blogger, YouTube)
                if (reqUrl.includes('googlevideo.com/videoplayback') || 
                    reqUrl.includes('youtube.googleapis.com') || 
                    (reqUrl.includes('mime=video') && reqUrl.includes('source=blogger'))) {
                    ...
                    // Try to extract real mime type from URL parameters
                    try {
                        const urlObj = new URL(reqUrl);
                        if (urlObj.searchParams.has('mime')) {
                            videoInfo.mimeType = urlObj.searchParams.get('mime');
                        }
                    } catch (e) {
                    ...
                    if (!videoUrls.some(v => v.url === reqUrl)) {
                        videoUrls.push(videoInfo);
                        util.Logging.info(`Detected Google video URL: ${reqUrl}`);
                    }
                }
                ...
                // Look for blob URLs which might be used for video streaming
                if (reqUrl.startsWith('blob:')) {
                    // Make note of blob URLs for debugging, though we can't directly use them
                    util.Logging.debug(`Detected blob URL: ${reqUrl}`);
                }
            });
            ...
            client.on('Network.responseReceived', async (event) => {
                try {
                    ...
                    const isGoogleVideo = responseUrl.includes('googlevideo.com') || 
                                         responseUrl.includes('youtube.googleapis.com');
                    ...
                    // Special case for Blogger embed URLs that redirect to actual videos
                    if (responseUrl.includes('blogger.com/video.g') && 
                        responseUrl.includes('token=')) {
                        // This is a Blogger video page which will redirect to the actual video
                        util.Logging.info(`Detected Blogger video URL: ${responseUrl}`);
                        
                        // We need to monitor redirects from this URL carefully
                        const videoInfo = {
                            url: responseUrl,
                            mimeType: 'video/mp4', // Default assumption
                            size: 0,
                            source: 'blogger_embed'
                        };
                        
                        // Track this URL to monitor for redirects
                        if (!videoUrls.some(v => v.url === responseUrl)) {
                            videoUrls.push(videoInfo);
                        }
                    }
        ...
        // Add a special handler function for Blogger videos within renderWithOptions

        // After navigation but before getting content, add this block for Blogger videos
        if (url.includes('blogger.com/video.g') && url.includes('token=')) {
            util.Logging.info(`Detected Blogger video URL, applying specialized handling: ${url}`);
            
            // Extend timeout for Blogger videos
            const bloggerVideoTimeout = Math.max(pageTimeout, 20000); // At least 20 seconds
            
            // Set a longer initial wait for the page to fully load
            await puppeteerDelay(5000);
            
            // Special interaction with Blogger video player
            const playButtonClicked = await page.evaluate(() => {
                let clicked = false;
                
                try {
                    // Try every possible play button and video element
                    // Blogger videos are typically in an iframe
                    const iframes = document.querySelectorAll('iframe');
                    
                    // First try to find the play button in the main document
                    const mainPlayButton = document.querySelector('[role="button"], .play-button, [aria-label="Play"]');
                    if (mainPlayButton) {
                        mainPlayButton.click();
                        clicked = true;
                        console.log('Clicked main play button');
                    }
                    
                    // Then try to access iframes if possible
                    iframes.forEach(iframe => {
                        try {
                            // Store iframe source for analysis
                            window.__bloggerIframeSources = window.__bloggerIframeSources || [];
                            if (iframe.src) {
                                window.__bloggerIframeSources.push(iframe.src);
                            ...
                            if (iframe.contentDocument) {
                                const iframePlayButtons = iframe.contentDocument.querySelectorAll(
                                    '[role="button"], .play-button, button'
                                );
                                ...
                                iframePlayButtons.forEach(button => {
                                    try { 
                                        button.click(); 
                                        clicked = true;
                                        console.log('Clicked iframe play button');
                                    } catch (e) {}
                                });
                                
                                // Try direct video elements
                                const videos = iframe.contentDocument.querySelectorAll('video');
                                videos.forEach(video => {
                                    try { 
                                        video.play().catch(() => {}); 
                                        clicked = true;
                                        console.log('Played iframe video');
                                    } catch (e) {}
                                });
                            }
                        } catch (e) {
                    ...
                    // Final attempt: click any element that looks interactive
                    if (!clicked) {
                        const possibleButtons = document.querySelectorAll('div, span, button, a');
                        possibleButtons.forEach(element => {
                            if (element.clientWidth > 20 && element.clientHeight > 20) {
                                try { 
                                    element.click(); 
                                    clicked = true;
                                    console.log('Clicked potential play area');
                                } catch (e) {}
                            }
                        });
                    }
                } catch (e) {
                    console.error('Error interacting with Blogger video:', e);
                }
            ...
            if (playButtonClicked) {
                util.Logging.info('Successfully clicked play button on Blogger video');
            } else {
                util.Logging.info('Could not find play button on Blogger video, trying alternate approaches');
            }
            
            // Wait for network requests that might contain the actual video
            await puppeteerDelay(5000);
            
            // Check if we already have a googlevideo URL from network traffic
            const hasGoogleVideo = videoUrls.some(v => 
                v.url.includes('googlevideo.com/videoplayback')
            );
            
            if (!hasGoogleVideo) {
                util.Logging.info('No Google Video URL detected yet, trying additional techniques');
                
                // Try to evaluate page again to find missed elements
                await clickVideoPlayButtons(page);
                
                // Wait a bit more for network requests
                await puppeteerDelay(3000);
                
                // If still no video URLs, try a direct fetch of the page to follow redirects
                if (!videoUrls.some(v => v.url.includes('googlevideo.com'))) {
                    try {
                        // Use CDP to make a request and follow redirects
                        const cdpResponse = await client.send('Network.request', {
                            url: url,
                            method: 'GET',
                            headers: {
                        ...
                        if (cdpResponse && cdpResponse.url && cdpResponse.url.includes('googlevideo.com')) {
                            util.Logging.info(`Found video URL through CDP request: ${cdpResponse.url}`);
                            videoUrls.push({
                                url: cdpResponse.url,
                                mimeType: 'video/mp4',
                                size: 0,
                        ...
                        util.Logging.warn(`Error making CDP request: ${e.message}`);
                    }
                }
            }
        }
