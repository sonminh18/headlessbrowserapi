    ...
    return page.evaluate(() => {
        ...
        const tryClick = (element, selector, isMp4 = false) => {
            try {
                ...
                // SAFETY CHECK: Universal check for navigation links
                // Prevent clicking any <a> tag that looks like a navigation link (not # or javascript:)
                if (tagName === 'A') {
                    if (href &&
                        !href.startsWith('#') &&
                        !href.startsWith('javascript:') &&
                        href.trim() !== '') {
                        console.log(`[VideoSource] SKIPPING unsafe navigation link: ${href}`);
                        return false;
                    }
                }
...
class PuppeteerSingleton {
    ...
    async renderWithOptions(req, res, next, options) {
        ...
        try {
            ...
            // Log the actual loaded URL to check for redirects
