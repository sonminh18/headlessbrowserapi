...
const isVideoUrl = (url) => {
    ...
    const hasVideoExtension = videoExtensions.some(extension => lowercasedUrl.endsWith(extension));
    
    // If not a direct video file extension, check if it's clearly a streaming URL
    const isStreamingUrl = lowercasedUrl.endsWith('.m3u8') || lowercasedUrl.endsWith('.mpd');
...
const extractVideoUrlsFromDOM = async (page) => {
    return page.evaluate(() => {
        ...
        const addUrl = (url, mimeType = '', size = 0) => {
            ...
            if (lowercasedUrl.match(/\.(jpe?g|png|gif|bmp|webp)(\?.*)?$/i)) {
                return;
            }
            ...
            if (isKnownVideoFormat || isVideoMimeType) {
                ...
                if (!mimeType) {
                    ...
                    else if (lowercasedUrl.endsWith('.mov')) mimeType = 'video/quicktime';
...
const getVideoMimeType = (url) => {
    const lowercasedUrl = url.toLowerCase().split('?')[0].split('#')[0];
    if (lowercasedUrl.endsWith('.mp4')) return 'video/mp4';
    if (lowercasedUrl.endsWith('.webm')) return 'video/webm';
    if (lowercasedUrl.endsWith('.ogg')) return 'video/ogg';
    if (lowercasedUrl.endsWith('.mov')) return 'video/quicktime';
    if (lowercasedUrl.endsWith('.avi')) return 'video/x-msvideo';
    if (lowercasedUrl.endsWith('.wmv')) return 'video/x-ms-wmv';
    if (lowercasedUrl.endsWith('.flv')) return 'video/x-flv';
    if (lowercasedUrl.endsWith('.mkv')) return 'video/x-matroska';
    if (lowercasedUrl.endsWith('.m4v')) return 'video/x-m4v';
    if (lowercasedUrl.endsWith('.mpeg') || lowercasedUrl.endsWith('.mpg')) return 'video/mpeg';
