...
const createAdminRouter = (options) => {
        ...
        try {
            ...
            for (const id of ids) {
                try {
                    ...
                    if (video.status === VIDEO_STATUS.SYNCED && deleteFirst && video.s3Key) {
                        try {
                            await s3Storage.deleteFile(video.s3Key);
                        } catch (deleteErr) {
                            logEmitter.warn(`Failed to delete S3 file ${video.s3Key}: ${deleteErr.message}`);
                        }
                    }
                    ...
                    await videoTracker.updateVideo(id, {
                        ...
                        error: null,
                        s3Url: force ? null : video.s3Url,
                        s3Key: force ? null : video.s3Key,
                        syncedAt: null
                    });
    ...
    router.post('/api/videos/:id/reupload', async (req, res) => {
        try {
            ...
            // If video is synced and deleteFirst is true, delete from S3 first
            if (video.status === VIDEO_STATUS.SYNCED && deleteFirst && video.s3Key) {
                try {
                    await s3Storage.deleteFile(video.s3Key);
            ...
            // Reset video status to pending for re-processing
            await videoTracker.updateVideo(id, {
                status: VIDEO_STATUS.PENDING,
                error: null,
                s3Url: force ? null : video.s3Url,
