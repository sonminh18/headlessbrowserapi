...
const createAdminRouter = (options) => {
    ...
    router.get("/api/dashboard", async (req, res) => {
        try {
            ...
            const browserStats = browserPool
     ...
     */
    router.get("/api/upload-queue/status", async (req, res) => {
        try {
            const {
                pendingPage = 1,
                pendingLimit = 20,
            ...
            // Add download queue stats
            // Add download queue jobs to the status response
            try {
                const { videoDownloadQueue } = require("../util/video-download-queue");

                const [waiting, active, failed, completed] = await Promise.all([
                    videoDownloadQueue.getWaiting(),
                    videoDownloadQueue.getActive(),
                    videoDownloadQueue.getFailed(),
                    videoDownloadQueue.getCompleted()
                ]);

                // Helper to map Bull job to UI format
                const mapJobToItem = (job, state) => ({
                    videoId: job.data.videoId,
                    videoUrl: job.data.url || `Video ${job.data.videoId}`,
                    sourceUrl: job.data.sourceUrl || "Auto Download",
                    state: state,
                ...
                // Merge into status
                const mappedActive = active.map(j => mapJobToItem(j, "active"));
                const mappedWaiting = waiting.map(j => mapJobToItem(j, "pending"));
                const mappedFailed = failed.map(j => mapJobToItem(j, "failed"));
                const mappedCompleted = completed.map(j => mapJobToItem(j, "completed"));

                // Combine lists
                status.active = [...status.active, ...mappedActive];

                // For pending, we append to existing pending
                status.pending = [...status.pending, ...mappedWaiting];
                status.pendingCount += mappedWaiting.length;
                status.pendingPagination.total += mappedWaiting.length;

                // For completed, we append/merge
                status.completed = [...status.completed, ...mappedCompleted, ...mappedFailed];
                // Sort by completedAt desc
                status.completed.sort((a, b) => new Date(b.completedAt || 0) - new Date(a.completedAt || 0));

                status.completedCount += mappedCompleted.length + mappedFailed.length;
                status.completedPagination.total += mappedCompleted.length + mappedFailed.length;

                status.activeCount += mappedActive.length;
                ...
                // If download queue not available, just log and skip
                console.error("Error fetching download queue stats:", err);
                status.downloadQueue = { waiting: 0, active: 0, failed: 0 };
            }
    ...
    /**
     * POST /admin/api/videos/:id/retry-source
     * Retry download with specific source index
     */
    router.post("/api/videos/:id/retry-source", async (req, res) => {
        try {
            const { id } = req.params;
            ...
            res.json({
                success: true,
                message: `Download job enqueued for source ${sourceIndex}`
            });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
