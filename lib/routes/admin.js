...
const createAdminRouter = (options) => {
     ...
     * List all videos with pagination, search, sorting
     */
     ...
     * Bulk re-upload multiple videos (synced or error status)
     */
    router.post('/api/videos/bulk-reupload', async (req, res) => {
        try {
            ...
            const results = await videoTracker.reuploadMany(ids);
            res.json(results);
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
     ...
     * For synced videos: Delete S3 file -> Download -> Upload
     * For error videos: Download -> Upload
     */
    router.post('/api/videos/:id/reupload', async (req, res) => {
        try {
            ...
            const result = await videoTracker.reuploadVideo(id);

            if (!result.success) {
                return res.status(result.error === 'Video not found' ? 404 : 400).json({ 
                    error: result.error 
                });
            }
