...
class BrowserPool {
    ...
    constructor(options = {}) {
        ...
        this.browserMaxPages = process.env.BROWSER_MAX_PAGES_PER_BROWSER ?
            parseInt(process.env.BROWSER_MAX_PAGES_PER_BROWSER)
            : 30;
        this.browserTTL = process.env.BROWSER_TTL ?
            parseInt(process.env.BROWSER_TTL)
    ...
    async getProcessInfo() {
        ...
        for (const [id, data] of this.browsers.entries()) {
            ...
            // Use pidusage to get actual memory and CPU usage
            if (pid) {
                try {
                    const stats = await pidusage(pid);
                    browserInfo.memory = {
                        rss: stats.memory,
                        rssMB: parseFloat((stats.memory / 1024 / 1024).toFixed(2))
                    };
                    browserInfo.cpu = parseFloat(stats.cpu.toFixed(2));
                } catch (err) {
                    // Process might have ended or stats unavailable
                    util.Logging.debug(`Could not get process stats for PID ${pid}: ${err.message}`);
                }
            }
