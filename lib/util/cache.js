...
class CacheManager {
    ...
    constructor(ttl = 3600) {
        ...
        util.Logging.info(`Initializing CacheManager with TTL: ${ttl}s`);
        ...
        if (this.redisEnabled) {
            util.Logging.info(`Cache mode: Redis primary with memory fallback (Redis URL: ${process.env.REDIS_URL || "redis://localhost:6379"})`);
        } else {
            util.Logging.info("Cache mode: Memory-only (Redis disabled)");
        }
        ...
        if (this.redisEnabled) {
            try {
                ...
                if (process.env.REDIS_PASSWORD) {
                    ...
                    util.Logging.info("Using Redis without password authentication");
                }
                ...
                this.redis.on("connect", () => {
                    ...
                    util.Logging.info("Connected to Redis cache successfully");
                });
