...
// Set log level from env: LOG_LEVEL=DEBUG|INFO|WARNING|ERROR|CRITICAL, or DEBUG=1 to enable debug
const logLevelEnv = (process.env.LOG_LEVEL || "").toUpperCase();
const debugEnv = process.env.DEBUG === "1" || process.env.DEBUG === "true";
const levelName = logLevelEnv || (debugEnv ? "DEBUG" : null);
if (levelName && Logging.constants[levelName] !== undefined) {
    Logging.getLogger("default").setLevel(Logging.constants[levelName]);
}
...
/**
 * Initialize S3 and upload configuration
 */
const start_S3 = () => {
    conf.S3 = {
    ...
    conf.UPLOAD = {
        maxSizeMB: parseInt(process.env.UPLOAD_MAX_SIZE_MB) || 500,
        maxConcurrentDownloads: parseInt(process.env.MAX_CONCURRENT_DOWNLOADS) || 2,
        timeout: parseInt(process.env.UPLOAD_TIMEOUT) || 300000
    };

    // yt-dlp download speed configuration
    conf.YTDLP = {
        // Number of fragments to download in parallel (1 = sequential, 4-8 recommended)
        concurrentFragments: parseInt(process.env.YTDLP_CONCURRENT_FRAGMENTS) || 4,
        // External downloader: "native" (yt-dlp built-in), "aria2c" (fastest, multi-connection)
        downloader: process.env.YTDLP_DOWNLOADER || "aria2c",
        // aria2c connections per server (only when downloader=aria2c)
        aria2cConnections: parseInt(process.env.YTDLP_ARIA2C_CONNECTIONS) || 16
    };
    ...
    conf.WATERMARK = {
        ...
        fontsize: parseInt(process.env.WATERMARK_FONTSIZE) || 24,
        opacity: parseFloat(process.env.WATERMARK_OPACITY) || 0.5,
        position: process.env.WATERMARK_POSITION || "bottom-right"
    };

    // Log S3 configuration (without secrets)
    Logging.debug(`S3 endpoint: ${conf.S3.endpoint || "not configured"}`);
    Logging.debug(`S3 bucket: ${conf.S3.bucket || "not configured"}`);
    Logging.debug(`S3 region: ${conf.S3.region}`);
    Logging.debug(`S3 path style: ${conf.S3.pathStyle}`);
    Logging.debug(`Upload max size: ${conf.UPLOAD.maxSizeMB}MB`);
    Logging.debug(`Max concurrent downloads: ${conf.UPLOAD.maxConcurrentDownloads}`);
    Logging.debug(`Upload timeout: ${conf.UPLOAD.timeout}ms`);
    Logging.debug(`Watermark: ${conf.WATERMARK.enabled ? `enabled ("${conf.WATERMARK.text}")` : "disabled"}`);
    Logging.debug(`yt-dlp: downloader=${conf.YTDLP.downloader}, concurrent_fragments=${conf.YTDLP.concurrentFragments}${conf.YTDLP.downloader === "aria2c" ? `, aria2c_connections=${conf.YTDLP.aria2cConnections}` : ""}`);
};
