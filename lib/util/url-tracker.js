...
class URLTracker {
    ...
    /**
     * Calculate duration in milliseconds for a record
     * @param {object} record - URL record
     * @returns {number} Duration in ms (0 if not calculable)
     * @private
     */
    _calculateDuration(record) {
     ...
     * @param {boolean} options.hasVideo - Filter URLs with videos
     * @param {string} options.sortBy - Field to sort by (createdAt, duration)
    ...
    async getAll(options = {}) {
        ...
        // Filter by hasVideo
        if (options.hasVideo === true || options.hasVideo === "true") {
            records = records.filter(r =>
                r.result?.videoUrls && r.result.videoUrls.length > 0
            );
        }
        ...
        records.sort((a, b) => {
            ...
            if (sortBy === "duration") {
                aVal = this._calculateDuration(a);
                bVal = this._calculateDuration(b);
