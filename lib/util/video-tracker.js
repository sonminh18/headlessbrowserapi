...
// Built-in ad/tracking URL patterns to filter out
const BUILTIN_AD_URL_PATTERNS = [
    /doubleclick\.net/i,
...
/**
 * Parse AD_URL_PATTERNS_EXTRA env var and append to built-in patterns.
 * Format: comma-separated list of regex-safe domain/pattern strings.
 * Example: AD_URL_PATTERNS_EXTRA="example\.com,badcdn\.net,adnetwork"
 * Each entry becomes a case-insensitive RegExp.
 */
function buildAdUrlPatterns() {
    const patterns = [...BUILTIN_AD_URL_PATTERNS];
    const extra = process.env.AD_URL_PATTERNS_EXTRA;
    if (extra) {
        extra.split(",")
            .map(s => s.trim())
            .filter(Boolean)
            .forEach(pattern => {
                try {
                    ...
                    util.Logging.warn(`Invalid AD_URL_PATTERNS_EXTRA pattern: "${pattern}"`);
                }
            });
        util.Logging.info(`AD_URL_PATTERNS: ${BUILTIN_AD_URL_PATTERNS.length} built-in + ${patterns.length - BUILTIN_AD_URL_PATTERNS.length} from env`);
    }
    return patterns;
}

const AD_URL_PATTERNS = buildAdUrlPatterns();
